-- Revisão e conclusão do Briefing com avanço atômico da Campanha.

alter table public.briefings_mediad
    add column alertas_reconhecidos jsonb not null default '[]'::jsonb
        check (jsonb_typeof(alertas_reconhecidos) = 'array');

create function public.transicionar_briefing_mediad(
...
begin
    ...
    if not (
        (v_antes.estado='EM_PREENCHIMENTO' and p_estado_destino='EM_REVISAO')
    ...
    if p_estado_destino='CONCLUIDO' then
...
revoke all on function public.transicionar_briefing_mediad(
    uuid, uuid, text, text, jsonb, uuid, timestamptz
) from public;
grant execute on function public.transicionar_briefing_mediad(
    uuid, uuid, text, text, jsonb, uuid, timestamptz
) to authenticated;
